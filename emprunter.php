<?php
// Démarre une nouvelle session ou récupère la session existante
session_start();
...
// Crée une instance de la classe Database pour gérer la connexion à la base de données
$db = new Database();
...
// Vérifie si un livre a été sélectionné (par l'ID dans l'URL)
if (!isset($_GET['id'])) {
    // Si aucun livre n'est sélectionné, redirige vers le tableau de bord utilisateur
...
// Prépare une requête SQL pour récupérer les informations du livre sélectionné
...
// Lie l'ID du livre à la requête SQL
...
// Si aucune information sur le livre n'est retournée, redirige vers le tableau de bord utilisateur
if (!$livre) {
    header("Location: dashboardu.php");
    exit();
}

// Prépare une requête SQL pour récupérer les informations de l'utilisateur à partir de son email
$sql = "SELECT id_utilisateur, prenom_utilisateur FROM utilisateur WHERE email_utilisateur = ?";
// Prépare la requête pour éviter les injections SQL
$stmt = $conn->prepare($sql);
// Lie l'email de l'utilisateur à la requête SQL
...
// Si aucune information sur l'utilisateur n'est retournée, redirige vers la page de connexion
if (!$user) {
    header("Location: connexion.php");
    exit();
}

// Récupère l'ID et le prénom de l'utilisateur
$id_utilisateur = $user['id_utilisateur'];
$prenom_utilisateur = $user['prenom_utilisateur'];
// Définit la date d'emprunt comme la date actuelle
$date_emprunt = date('Y-m-d');
// Définit la date de retour comme 14 jours après la date d'emprunt
